...
export const DiffView = ({ oldCode, newCode, languageId, }) => {
    const { grammar, language } = toPrismHighlightOptions(languageId);
    const { highlight } = getPrism();
    return (React.createElement(ReactDiffViewer, { oldValue: oldCode, newValue: newCode, splitView: true, showDiffOnly: true, extraLinesSurroundingDiff: 3, compareMethod: DiffMethod.WORDS, renderContent: (str) => {
            return (React.createElement("pre", { style: { display: "inline" }, dangerouslySetInnerHTML: {
